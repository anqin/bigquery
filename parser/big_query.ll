...
#include "bigquery/parser/big_query.yy.hh"

#define YY_USER_ACTION \
        bigquery_lloc.first_column = yycolumn; \
        bigquery_lloc.last_column = yycolumn + yyleng - 1; \
        yycolumn += yyleng;

#define YY_USER_INIT bigquery_lloc.first_line = 0; \
                     bigquery_lloc.last_line = 0; \
                     bigquery_lloc.first_column = 0; \
                     bigquery_lloc.last_column = 0
...
namespace dc = ::bigquery::compiler;
...
TRUE {
    bigquery_lval._bool = true;
    return BOOLEAN;
}

FALSE {
    bigquery_lval._bool = false;
    return BOOLEAN;
}
...
({id}+{digit}*)+ {
    ...
    *(m->mutable_char_string()) = std::string(yytext);
    bigquery_lval._string = m;
    return ID;
}

{digit}+ {
    bigquery_lval._int = atol(yytext);
    return INT;
}

{digit}+\.{digit}+ {
    bigquery_lval._float = atof(yytext);
    return FLOAT;
}
