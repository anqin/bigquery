...
#include "bigquery/parser/proto_message.h"
...
bool ProtoMessage::CreateMessageByProtoFile(const std::string& proto_file,
    ...
    const std::string local_proto_file = proto_file; // MoveDfsFileToLocalTmp(proto_file);
    if (local_proto_file == "") {
        return false;
    }
    m_message = NULL;
...
//     m_source_tree->MapPath("", ::io::path::GetDirectory(local_proto_file));
    m_source_tree->MapPath("", local_proto_file);

    m_file_descriptor =
//         m_importer->Import(::io::path::GetBaseName(local_proto_file));
        m_importer->Import(local_proto_file);
